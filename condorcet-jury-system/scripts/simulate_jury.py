#!/usr/bin/env python3
"""
Condorcet Jury System - Simulated Evaluation

Demonstrates the jury system without API calls.
Uses probabilistic modeling based on persona characteristics.

Usage:
    # Simple content evaluation
    python simulate_jury.py --content "Your product idea here"
    
    # With initiative folder
    python simulate_jury.py --initiative path/to/initiative --phase research --jury-size 100
"""
...
def load_personas(path: Optional[Path] = None) -> List[Dict]:
    """Load personas from JSON file."""
    if path is None:
        path = get_project_root() / "personas" / "generated" / "batch-2026-01-08" / "all-personas.json"
    
    if not path.exists():
        # Try alternate paths
        alt_paths = [
            get_project_root() / "personas" / "generated" / "batch-2026-01-09" / "personas.json",
            get_project_root() / "personas" / "archetypes",
        ]
        for alt in alt_paths:
            if alt.exists():
                if alt.is_dir():
                    # Load all archetypes and generate synthetic pool
                    return generate_synthetic_personas(alt)
                else:
                    path = alt
                    break
        else:
            print("⚠️ No persona files found. Generating minimal synthetic personas...")
            return generate_minimal_personas()
    
    with open(path) as f:
        return json.load(f)


def generate_minimal_personas() -> List[Dict]:
    """Generate minimal personas when no files exist."""
    ...
    for i in range(100):
        ...
        adoption = random.choices(adoptions, weights=[0.35, 0.25, 0.25, 0.15])[0]
        personas.append({
            "id": f"generated-{i}",
            "archetype_id": arch,
            "psychographics": {
...
def generate_synthetic_personas(archetype_dir: Path) -> List[Dict]:
    """Generate personas from archetypes."""
    personas = []
    adoptions = ["skeptic", "curious", "early-adopter", "power-user"]
    
    for arch_file in archetype_dir.glob("*.json"):
        ...
        for i in range(50):
            adoption = random.choices(adoptions, weights=[0.35, 0.25, 0.25, 0.15])[0]
            persona = {
                "id": f"{archetype.get('archetype', 'unknown')}-{i}",
                "archetype_id": archetype.get("archetype", arch_file.stem),
                "psychographics": {
                    "ai_adoption_stage": adoption,
                    "trust_in_ai": random.uniform(0.3, 0.8),
                    "tool_fatigue": random.uniform(0.2, 0.7),
...
def simulate_persona_evaluation(persona: Dict, content: str, phase: str) -> Dict:
    ...
    # Generate concerns based on adoption stage
    concern_pools = {
        "skeptic": [
    ...
    pool = concern_pools.get(adoption, concern_pools["curious"])
    concerns = random.sample(pool, min(2, len(pool)))
    
    # Generate suggestions based on archetype
    ...
    sugg_pool = suggestion_pools.get(archetype, suggestion_pools["sales-rep"])
    suggestions = random.sample(sugg_pool, min(2, len(sugg_pool)))
    ...
    voice_templates = {
        "skeptic": [
            "Look, I've been burned by these AI tools before. I need to see exactly what it's doing before I trust it.",
            "My team has tool fatigue. Unless this is absolutely seamless, they won't use it.",
            "I'm not opposed to AI, I'm opposed to hype. Show me the ROI first.",
        ],
        "curious": [
            "This sounds promising, but I need to understand the setup process better.",
            "I like the vision here. My concern is whether my team will actually adopt it.",
            "Interesting approach. I'd want to pilot this with a small group first.",
        ],
        "early-adopter": [
            "This is exactly what I've been looking for. The transparency features are key.",
            "Love the direction. Just need better preview/testing capabilities.",
            "The pain points you've identified are spot-on. If the execution matches, I'm in.",
        ],
        "power-user": [
            "Good start, but I need more control. Let me customize the exact behavior.",
            "The core concept is solid. I want API access so I can build automations around this.",
            "This would save me hours if I can configure it exactly the way I need.",
        ],
    }
    ...
    # Resonance score
    resonance = random.randint(5, 10) if verdict == "approve" else (
        random.randint(4, 7) if verdict == "conditional" else random.randint(2, 5)
    )
...
def sample_stratified_jury(personas: List[Dict], size: int = 100, skeptic_target: float = 0.20) -> List[Dict]:
    ...
    by_adoption = {"skeptic": [], "curious": [], "early-adopter": [], "power-user": []}
    for p in personas:
...
{"✅ The jury strongly supports this initiative." if agg['approval_rate'] > 50 else "⚠️ The jury has conditional support. Address key concerns." if combined >= pass_threshold else "❌ The jury does not support this initiative in its current form."}
...
- **Skeptic Pass Rate:** {skeptic_pass}%
- **Skeptic Count:** {skeptic_total} personas

Key skeptic themes:
- Need proof of ROI before commitment
- Fear of tool fatigue
...
"""
    for i, f in enumerate(agg["friction_points"][:5], 1):
        report += f"{i}. **{f['theme']}** - {f['count']} personas\n"
...
1. **Visibility** - Show what will happen before it happens
2. **Testing** - Sandbox/preview mode
3. **Rollback** - Easy undo

### Should Have
4. **Activity Log** - Audit trail
5. **Templates** - Pre-built configurations
6. **Gradual Rollout** - Pilot capability

### Nice to Have
7. **API Access** - Custom workflows
8. **Mobile View** - Status monitoring
9. **ROI Dashboard** - Time savings metrics
...
"""
        ...
        report += """1. Revisit problem framing based on feedback
2. Interview real skeptic personas
3. Consider pivoting approach
"""
...
def main():
    parser = argparse.ArgumentParser(description="Condorcet Jury System - Simulated Evaluation")
    parser.add_argument("--content", type=str, help="Content to evaluate (product idea, feature description)")
    parser.add_argument("--initiative", type=str, help="Path to initiative folder with research.md")
    parser.add_argument("--phase", default="research", choices=["research", "prd", "prototype"])
    parser.add_argument("--jury-size", type=int, default=100)
    parser.add_argument("--skeptics", type=float, default=0.20, help="Target percentage of skeptics (0.0-1.0)")
    ...
    if args.initiative:
        ...
        initiative_name = initiative_path.name
    elif args.content:
        initiative_name = "content-evaluation"
    else:
        parser.error("Either --content or --initiative is required")
        return
    ...
    # Sample jury
    jury = sample_stratified_jury(personas, args.jury_size, args.skeptics)
    ...
    if args.initiative:
        research_file = Path(args.initiative) / "research.md"
        if research_file.exists():
            content = research_file.read_text()
    
    # Run evaluations
    print(f"🧪 Running {len(jury)} simulated evaluations...")
    results = [simulate_persona_evaluation(p, content, args.phase) for p in jury]
    ...
    report = generate_report(aggregated, initiative_name, args.phase)
    
    # Determine output directory
    if args.output:
        output_dir = Path(args.output)
    elif args.initiative:
        output_dir = Path(args.initiative) / "jury-evaluations"
    else:
    ...
    latest_data = output_dir / "latest-eval.json"
