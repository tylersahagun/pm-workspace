...
| 5 | **Inline column configuration** | Click column header → popover with prompt, output type, conditions (Notion pattern) |
...
### NEW: Configuration UX Requirements (from Notion/Clay Research)

| # | Requirement | Acceptance Criteria | Reference |
|---|-------------|---------------------|-----------|
| 9 | **One-click column creation** | Click "+" → Select type → Column exists (< 3 seconds) | Notion "+" pattern |
| 10 | **Auto-preview** | Preview runs automatically on 3 rows after prompt entry (500ms debounce) | Clay preview, faster |
| 11 | **Template quick-starts** | Inline chips for common columns; one click applies prompt | Notion templates |
| 12 | **Direct manipulation** | Drag to reorder columns, click to edit inline, no save button | Notion everywhere |
| 13 | **Evidence receipts** | Click any AI cell to see source quotes that triggered the extraction | Trust pattern |
| 14 | **Cell-level feedback** | Thumbs up/down on individual extractions; manual override option | Trust recovery |

### Should Have (Fast Follow)

| # | Requirement | Notes |
|---|-------------|-------|
| 15 | Column templates | Pre-built common extractions (competitors, objections, next steps) |
| 16 | Graduate to signal | Promote ad-hoc column to ongoing signal extraction |
| 17 | Entity grouping | Group engagements by company for roll-up view |
| 18 | Export to CSV | Basic data export for external analysis |
| 19 | **Keyboard shortcuts** | Cmd+K for new column, arrow keys for navigation, Enter to run |

### Could Have (v2+)

| # | Requirement | Notes |
|---|-------------|-------|
| 20 | Companies/Deals as row entity | Full entity-level tables |
| 21 | External enrichment | Web scraping, CRM data pulls |
| 22 | Dashboard/graph creation | Visual outputs from table data |
| 23 | Workflow triggers | "If column = X, trigger action Y" |
| 24 | **User-created templates** | Save column configs as reusable templates |
...
| Notion | Flexible databases; no AI extraction | AI-powered extraction from conversations |

**Our angle:** "Clay for your own conversations" — but simpler, faster, and no scary complexity.

### UX Positioning (from Configuration Research)

| Dimension | Clay | Notion | AskElephant Signal Tables |
|-----------|------|--------|---------------------------|
| **Primary metaphor** | Workflow orchestrator | Spreadsheet you enhance | AI-powered conversation explorer |
| **Data source** | 100+ external APIs | User-entered | Our transcripts (we own it) |
| **Configuration mode** | Explicit wizard | None (inline everywhere) | **Inline popovers (Notion pattern)** |
| **AI role** | Column logic | Background (formulas) | **Foreground + transparent** |
| **Learning curve** | Steep (powerful) | Gentle (familiar) | **Gentle (our goal)** |
| **Token visibility** | Prominent (anxiety) | N/A | **Hidden until relevant** |

**Why we can be simpler than Clay:**
We're not orchestrating external APIs. We're querying data we already have. One data source = fewer configuration decisions = faster time to insight.
...
- **Design Brief:** `design-brief.md` (updated with Configuration UX paradigm)
- **Configuration UX Research:** `research/configuration-ux-patterns.md` (Notion/Clay analysis)
- **Prototype Notes:** `prototype-notes.md` (implementation guidance)

---

*Last updated: 2026-01-09*
*Owner: Tyler, Skyler*
*Configuration UX Research: Jan 9, 2026*